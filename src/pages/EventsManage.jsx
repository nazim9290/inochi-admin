...
const SEED_EVENTS = [
  {
    type: 'seminar',
    title: 'ফ্রি জাপান স্টাডি সেমিনার — মে ২০২৬',
    titleEn: 'Free Japan Study Seminar — May 2026',
    titleJa: '無料日本留学セミナー — 2026年5月',
    description:
      'বাংলাদেশ→জাপান path-এর overview, সাম্প্রতিক ভিসা আপডেট ও Inochi counsellor-দের সাথে Q&A।',
    descriptionEn:
      'Overview of the Bangladesh→Japan pathway, recent visa updates, and Q&A with Inochi counsellors.',
    descriptionJa: 'バングラデシュ→日本ルートの概要、最新ビザ情報、Inochi カウンセラーによる Q&A。',
    eventDate: '2026-05-15',
    time: '16:00',
    durationMin: 90,
    location: 'Inochi ঢাকা অফিস (ও অনলাইন)',
    locationEn: 'Inochi Dhaka office (and online)',
    locationJa: 'Inochi ダッカ事務所 + オンライン',
    city: 'dhaka',
    rsvpUrl: '/seminars',
    isFree: true,
    highlight: true,
  ...
  {
    type: 'workshop',
    title: 'JLPT N5 ক্র্যাশ ওয়ার্কশপ — ১ দিনে hiragana',
    titleEn: 'JLPT N5 Crash Workshop — Hiragana mastery in a day',
    ...
    time: '10:00',
    durationMin: 240,
    location: 'Inochi ঢাকা অফিস',
    locationEn: 'Inochi Dhaka office',
    locationJa: 'Inochi ダッカ事務所',
    city: 'dhaka',
    rsvpUrl: '/contact',
    fee: '৳500',
    sortOrder: 1,
  ...
  {
    type: 'meetup',
    title: 'বাংলাদেশ × জাপান আলামনাই মিটআপ — সাইতামা',
    titleEn: 'Bangladesh × Japan Alumni Meetup — Saitama',
    titleJa: 'バングラデシュ × 日本 卒業生ミートアップ — 埼玉',
    description:
      'জাপানে অবস্থানরত Inochi alumni চা-পান করতে জড়ো — কাজ-খোঁজা ও ভিসা-extension অভিজ্ঞতা share।',
    descriptionEn:
      'Inochi alumni currently in Japan gather over chai, share job-hunt and visa-extension experiences.',
    descriptionJa:
      '日本在住の Inochi 卒業生がチャイを囲んで集合 — 就職活動とビザ更新の経験を共有。',
    eventDate: '2026-06-05',
    time: '14:00',
    durationMin: 180,
    location: 'Inochi সাইতামা অফিস',
    locationEn: 'Inochi Saitama office',
    locationJa: 'Inochi 埼玉事務所',
    city: 'saitama',
    rsvpUrl: '/contact',
    isFree: true,
    sortOrder: 2,
  ...
  {
    type: 'festival',
    title: 'সাকুরা উৎসব Watch Party 🌸',
    titleEn: 'Sakura Festival Watch Party 🌸',
    titleJa: '桜祭りウォッチパーティー 🌸',
    description:
      'হানামি (花見) সংস্কৃতি — sakura সিজনে জাপানি পরিবারের রীতি, খাবার ও পার্কে time spend।',
    descriptionEn:
      'Hanami (flower viewing) culture — Japanese family rituals during sakura season, food, and park time.',
...
export default function EventsManage() {
  ...
  const onChange = (e) => {
    ...
    setForm((p) => ({
      ...
      [name]:
        type === 'checkbox'
          ? checked
          : name === 'sortOrder' || name === 'durationMin'
            ? Number(value)
            : value,
    }));
  };
  ...
  const remove = async (id) => {
    const ok = await confirmDialog({
      title: 'এই event delete?',
      message: '/events পেজ থেকে সরে যাবে।',
    ...
    <div className="space-y-6 max-w-5xl">
      <div>
        ...
        <p className="mt-1 text-sm text-brand-slate">
          /events পেজে যে seminar, workshop, alumni meetup, উৎসব দেখানো হয়। নতুন event যোগ করলে ৬০
          সেকেন্ডে public site-এ দেখা যাবে।
        </p>
      </div>
      ...
      {isEmpty && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          ...
          <p className="mt-1 leading-relaxed">
            এক click-এ পুরাতন চারটি sample event (seminar, workshop, meetup, sakura festival) import
            করতে পারেন।
          </p>
          <button
            type="button"
      ...
      <PasteFromClaude type="event" onApply={(f) => setForm((p) => ({ ...p, ...f }))} />
      <form
        onSubmit={submit}
        className="space-y-5 rounded-xl border border-brand-tealLight/40 bg-white p-5 shadow-sm"
      >
        <h2 className="text-sm font-bold uppercase tracking-wide text-brand-navy">
          {editingId ? 'Event Edit' : 'নতুন Event যোগ'}
        </h2>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <label>
            <span className={labelClass}>তারিখ</span>
            <input
              type="date"
              name="eventDate"
          ...
          <label>
            <span className={labelClass}>সময় (24-hr)</span>
            <input
              type="time"
              name="time"
              value={form.time}
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <label>
            <span className={labelClass}>Title (BN)</span>
            <input
              name="title"
              value={form.title}
              onChange={onChange}
              className={inputClass}
              required
              placeholder="ফ্রি জাপান সেমিনার"
            />
          </label>
          <label>
            ...
            <input
              ...
              onChange={onChange}
              className={inputClass}
              placeholder="Free Japan Seminar"
            />
          </label>
          <label>
            <span className={labelClass}>タイトル (JA)</span>
            <input name="titleJa" value={form.titleJa} onChange={onChange} className={inputClass} />
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <label>
            <span className={labelClass}>Location (BN)</span>
            <input
              name="location"
              value={form.location}
              onChange={onChange}
              className={inputClass}
              placeholder="Inochi ঢাকা অফিস"
            />
          </label>
          <label>
            ...
            <input
              ...
              onChange={onChange}
              className={inputClass}
              placeholder="Inochi Dhaka office"
            />
          </label>
          <label>
            <span className={labelClass}>場所 (JA)</span>
            <input
              name="locationJa"
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
          <label>
            <span className={labelClass}>City slug (filter)</span>
            <input
              name="city"
              value={form.city}
              onChange={onChange}
          ...
          <label>
            <span className={labelClass}>সময়কাল (মিনিট)</span>
            <input
              type="number"
              name="durationMin"
          ...
          <label className="md:col-span-2">
            <span className={labelClass}>RSVP URL</span>
            <input
              name="rsvpUrl"
              value={form.rsvpUrl}
              onChange={onChange}
              className={inputClass}
              placeholder="/contact বা https://..."
            />
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <label className="flex items-end gap-2 pb-2">
            ...
            <span className="text-xs font-semibold text-brand-navy">ফ্রি event</span>
          </label>
          <label>
            <span className={labelClass}>ফি (যদি ফ্রি না হয়)</span>
            <input
              name="fee"
              value={form.fee}
              onChange={onChange}
          ...
          <label className="flex items-end gap-2 pb-2">
            ...
            <span className="text-xs font-semibold text-brand-navy">⭐ উপরে pin করুন</span>
          </label>
        </div>
        ...
        <div className="flex gap-2 pt-2">
          <button
            type="submit"
            disabled={busy}
            className="rounded bg-brand-teal px-5 py-2 text-sm font-semibold text-white hover:bg-brand-navy disabled:opacity-50"
          >
          ...
          {editingId && (
            <button
              type="button"
              onClick={reset}
              className="rounded border border-brand-navy bg-white px-5 py-2 text-sm font-semibold text-brand-navy"
            >
              Cancel
            </button>
          )}
      ...
      {loading ? (
        ...
        <ul className="space-y-3">
          {events.map((e) => (
            <li
              key={e.id}
              className="flex items-start justify-between gap-4 rounded-xl border border-brand-tealLight/40 bg-white p-4 shadow-sm"
            >
              <div className="flex-1">
                ...
                <p className="mt-1 text-xs text-brand-slate">
                  {e.eventDate} {e.time && `· ${e.time}`} · {e.location || '—'}
                </p>
              </div>
              <div className="flex flex-shrink-0 gap-2 text-xs font-semibold">
                <button onClick={() => edit(e)} className="text-brand-teal hover:text-brand-navy">
                  Edit
                </button>
